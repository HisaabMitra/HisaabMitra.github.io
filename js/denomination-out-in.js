...
window.DenominationOutInComponent = {
                ...
                <tbody>
                    ${notes.map((note, index) => `
                        <tr style="border-bottom: 1px solid #f6f6f6;">
                            ...
                            <td style="padding:8px;"><input type="number" class="gen-out-val" data-note="${note}" value="0" min="0" tabindex="${30 + index}" style="width:70px; padding:6px; text-align:center; border:1px solid #dcdcdc; border-radius:4px; font-weight:600;"></td>
                            <td style="padding:8px; font-size:0.9rem; color:#c0392b; font-weight:700;" id="gen-out-display-${note}">₹0</td>
                        </tr>
                    `).join('')}
    ...
    calculate: function() {
        ...
        notes.forEach(note => {
            const inputOut = document.querySelector(`.gen-out-val[data-note="${note}"]`);
            const display = document.getElementById(`gen-out-display-${note}`);
            if (inputOut) {
        ...
        const coinsInput = document.querySelector('.gen-out-val[data-note="coins"]');
        const coinsDisplay = document.getElementById('gen-out-display-coins');
        if (coinsInput) {
        ...
        const totalCalculated = document.getElementById('gen-out-total-calculated');
        if (totalCalculated) totalCalculated.innerText = `₹${grandTotal}`;
        return grandTotal;
    },

    attachListeners: function() {
        document.querySelectorAll('.gen-out-val').forEach(input => {
    ...
    getValues: function() {
        ...
        [500, 200, 100, 50, 20, 10, 5].forEach(note => {
            const el = document.querySelector(`.gen-out-val[data-note="${note}"]`);
            notesData[`cash_${note}`] = el ? (parseInt(el.value) || 0) : 0;
        });
        const coinsEl = document.querySelector('.gen-out-val[data-note="coins"]');
        notesData['cash_coins'] = coinsEl ? (parseInt(coinsEl.value) || 0) : 0;
        return notesData;
    },

    clear: function() {
        document.querySelectorAll('.gen-out-val').forEach(input => input.value = 0);
        this.calculate();
    }
};
